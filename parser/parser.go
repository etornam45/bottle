...
func (p *Parser) peekPrecedence() int {
	if p, ok := precedences[p.tokenAhead.Type]; ok {
		return p
	}
	return LOWEST
}
...
func (p *Parser) nextToken() {
	p.curToken = p.tokenAhead
	p.tokenAhead = p.l.Next()
}
...
func (p *Parser) tokenAheadIs(t token.TokenType) bool {
	return p.tokenAhead.Type == t
}

func (p *Parser) expectAhead(t token.TokenType) bool {
	if p.tokenAheadIs(t) {
		p.nextToken()
		return true
	} else {
...
func (p *Parser) errorAhead(t token.TokenType) {
...
func (p *Parser) parseLetStatement() *ast.LetStatement {
	...
	if !p.expectAhead(token.IDENT) {
		return nil
	}

	stmt.Name = &ast.Identifier{Token: p.curToken, Value: p.curToken.Literal}

	if !p.expectAhead(token.ASSIGN) {
		return nil
	}
...
func (p *Parser) parseArrayLiteral() ast.Expression {
  ...
  for p.curToken.Type != token.RBRACKET {
    ...
    // println(p.curToken.Literal, p.curToken.Type)
    p.nextToken() //TODO: Raise an errors if next token not ,
    // println(p.curToken.Literal, p.curToken.Type)
  }
  return arr
}
...
func (p *Parser) parseExpressionStatement() *ast.ExpressionStatement {
	...
	if p.tokenAheadIs(token.SEMICOLON) {
		p.nextToken()
	}
...
func (p *Parser) parseExpression(precedence int) ast.Expression {
	...
	for !p.tokenAheadIs(token.SEMICOLON) && precedence < p.peekPrecedence() {

		infix := p.infixParserFns[p.tokenAhead.Type]
...
func (p *Parser) parseIntegerLiteral() ast.Expression {
	...
	if err != nil {
		...
		mgs := fmt.Sprintf("could not parse %q as integer", p.curToken.Literal)
		p.errors = append(p.errors, mgs)
